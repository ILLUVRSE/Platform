...
import { generateLiveLoopSchedule, mapLiveLoopEvents, type LiveLoopSlot } from "./utils";
...
export default function LiveLoopRokuPage() {
  ...
  const { schedule: inkedSlots, onAirSlot, nextSlot } = useMemo(
    () => mapLiveLoopEvents(slots, nowMinutes),
    [slots, nowMinutes]
  );
  ...
  const activeSlot = onAirSlot ?? inkedSlots[0];
  const onAirTitle = activeSlot?.title ?? "Live";
