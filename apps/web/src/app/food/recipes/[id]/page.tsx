...
import { Recipe, GroceryItem } from '@food/lib/types';
import { ArrowLeft, ShoppingBasket, Users, Trash2, Share2, Printer, Play, Pause, PenLine, Star } from 'lucide-react';
import Link from 'next/link';
import { ChatInterface } from '@food/components/ChatInterface';
import { categorizeItem, formatQuantity } from '@food/lib/utils';
...
export default function RecipeDetailPage() {
  ...
  const shareUrl =
    recipe.shareId && typeof window !== 'undefined'
      ? `${window.location.origin}/food/recipes/share/${recipe.shareId}`
      : null;
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div className="lg:col-span-2 space-y-8">
        <div className="flex items-center justify-between">
            <Link href="/food" className="inline-flex items-center text-stone-500 hover:text-primary transition-colors">
            <ArrowLeft className="w-4 h-4 mr-2" /> Back to Recipes
             ...
             <div className="flex items-center gap-3">
                ...
                <Link
                  href={`/food/recipes/new?edit=${recipe.id}`}
                  className="inline-flex items-center gap-1 text-sm px-3 py-1.5 rounded-full border border-stone-200 hover:border-primary hover:text-primary"
                >
                  <PenLine className="w-4 h-4" /> Edit
                </Link>
                <Link
                  href={`/food/recipes/${recipe.id}/print`}
                  className="inline-flex items-center gap-1 text-sm px-3 py-1.5 rounded-full border border-stone-200 hover:border-primary hover:text-primary"
                >
