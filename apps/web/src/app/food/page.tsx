...
import { RecipeThumbnail } from '@food/components/RecipeThumbnail';
...
export default function Home() {
  ...
  const formatCookedLabel = (timestamp?: number) => {
    ...
    if (diffDays < 7) return `Cooked ${diffDays}d ago`;
    return `Cooked ${new Date(timestamp).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}`;
  };

  const filtered = useMemo(() => {
    const matches = recipes.filter((r) => {
      if (favoriteOnly && !r.favorite) return false;
      if (tagFilters.length && !(r.tags || []).some((tag) => tagFilters.includes(tag))) return false;
  ...
  }, [recipes, query, favoriteOnly, tagFilters, sortBy, lastCookedLookup]);
  ...
  const hasActiveFilters = Boolean(query || favoriteOnly || tagFilters.length || sortBy !== 'priority');
      ...
      <div className="bg-white border border-stone-200 rounded-2xl shadow-sm p-5 space-y-4">
        <div className="flex flex-wrap gap-3 items-center">
          ...
          <button
            ...
            className="text-xs px-3 py-2 rounded-full border border-stone-200 hover:border-primary hover:text-primary disabled:opacity-50"
        ...
        <div className="flex flex-wrap items-center gap-2 text-sm text-stone-600">
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1 rounded-full border text-xs font-semibold transition-colors ${
                      active
                        ? 'bg-amber-50 text-secondary border-amber-200'
          ...
          {tagFilters.length > 0 && (
            <button
              ...
              className="text-xs px-3 py-1 rounded-full border border-stone-200 hover:border-primary hover:text-primary"
            >
              Clear tags
            </button>
          )}
      ...
      {user ? (
        recipes.length === 0 ? (
          ...
          <div className="text-center py-16 bg-white rounded-2xl border border-dashed border-stone-300 shadow-sm">
            ...
            <p className="text-stone-500 mb-6">Try clearing filters or using a different search.</p>
            <button
              type="button"
              onClick={() => {
                setQuery('');
            ...
            {filtered.map((recipe) => (
              ...
              >
                ...
                <div className="p-5 space-y-3">
                  ...
                  <p className="text-xs text-stone-400">{formatCookedLabel(lastCookedLookup[recipe.id])}</p>
                </div>
              </Link>
            ))}
          </div>
