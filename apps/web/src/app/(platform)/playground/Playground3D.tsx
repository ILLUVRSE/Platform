...
export function Playground3D({ handoffManifest }: { handoffManifest?: AceAgentManifest | null }) {
  const [selected, setSelected] = useState<AceAgentManifest | null>(null);
  const [customManifest, setCustomManifest] = useState<AceAgentManifest | null>(null);
  ...
  function applyStoredManifest(parsed: AceAgentManifest, label: string) {
    ...
    setToast(`Selected ${parsed.name} from Playground manifest`);
    setTimeout(() => setToast(null), 2500);
  }

  // Auto-select manifest from handoff or local storage
  useEffect(() => {
    if (handoffManifest) {
      applyStoredManifest(handoffManifest, "loaded from handoff");
      return;
    }
    try {
      ...
      applyStoredManifest(parsed, "loaded from Playground");
    } catch {
      // ignore
    }
  }, [handoffManifest]);
  ...
  const customPosition: [number, number, number] = [0, 0.9, 0];
  ...
  function handleSelect(manifest: AceAgentManifest) {
    ...
    const isTutorial = tutorialManifests.some((item) => item.manifest.id === manifest.id);
    if (isTutorial) {
      setCustomManifest(null);
    } else {
      ...
      <div className="h-[420px] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-card">
        <Canvas camera={{ position: [0, 4, 8], fov: 50 }}>
          ...
          <XR enabled={xrEnabled}>
            ...
            {customManifest ? (
              <AgentNode
                key={customManifest.id}
                manifest={customManifest}
                position={customPosition}
                onSelect={handleSelect}
                selected={selected?.id === customManifest.id}
                statusText={statusMap[customManifest.id]?.text}
                statusRaw={statusMap[customManifest.id]?.status}
                proofSha={statusMap[customManifest.id]?.proofSha}
