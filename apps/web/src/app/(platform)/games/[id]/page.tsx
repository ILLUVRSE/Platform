...
export function generateMetadata({ params }: { params: { id: string } }): Metadata {
  const game = gameGridManifest.find((item) => item.id === params.id);
  if (!game) {
    return buildMetadata({
      title: "GameGrid | ILLUVRSE",
      description: "GameGrid experiences inside ILLUVRSE.",
      path: `/games/${params.id}`,
      noIndex: true
    });
  }

  const title = `${game.title} | GameGrid`;
  return buildMetadata({
    title,
...
export default function GamePage({ params }: { params: { id: string } }) {
  ...
  const pageJsonLd = buildJsonLd({
    title: `${game.title} | GameGrid`,
    description: game.description,
    path: `/games/${params.id}`,
    type: "VideoGame",
    extra: { genre: game.genre }
  });
  return (
    <>
